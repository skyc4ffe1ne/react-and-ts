export function calculateLines(
  ...
  if (!flagColor) {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (i === maxRow) {
        finalString += `<span style=color:${arr[i].color}>${arr[i].char}</span><\br>`;
      } else {
        finalString += `<span style=color:${arr[i].color}>${arr[i].char}</span>`;
      }
    }
  }
...
export function asciiColors(stringAscii: string) {
  const arrAscii = [stringAscii];
...
export function getAscii(
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (flagColor) {
      asciiCharArr.push(`<span>${getChar(lightness)}</span>`);
    } else {
